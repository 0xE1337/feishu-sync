...
**含义**：wiki_token 或 obj_token 对应的资源不存在（拼错了，或你没有权限看到它——飞书把"无权限"也会包装成"不存在"）。

**修法**：
1. 确认 URL 拼写：`.../wiki/<token>` 里的 token 必须是完整的 27 字符字符串
2. 在本人已登录的浏览器里**能不能打开这个 URL**？不能就是资源真不存在或你个人也没权限
3. 本人能开但 API 失败：**不是** `131005`，是 `131006`（权限）——再看一眼错误码
